...
class ChannelUserController extends Controller
{
    ...
    public function addUserToChannel($id){
     ...
     public function deleteUserFromChannel($id){
         ...
         try {
          Log::info('Delete user from channel');
          ...
          return response()->json([
              'success' => true,
              'message' => 'Delete user from channel',
              'data' => $user
          ], 200);
  
         } catch (\Exception $exception) {
          Log::error('Error Deleting user from channel: ' . $exception->getMessage());
          return response()->json([
              'success' => true,
              'message' => 'Unable to Delete user from channel',
              'data' => $user
          ], 500);
         }
      }
